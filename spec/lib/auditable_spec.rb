...
describe Auditable do
  ...
  it "should work when we have multiple audits created per second (same created_at timestamps)" do
    require 'timecop'
    Timecop.freeze do
      survey.update :title => "new title 1"
      survey.update :title => "new title 2"
      survey.audited_changes.should == {"title" => ["new title 1", "new title 2"]}
      survey.update :title => "new title 3"
      survey.update :title => "new title 4"
      survey.update :title => "new title 5"
    ...
    describe ".audited_changes" do
      context "using :tag and :changed_by arguments" do
        before do
          ...
          survey.update :title => "new title 1", :changed_by => user
          survey.audit_tag_with("locked")
          survey.update :title => "new title 2", :changed_by => another_user
          survey.audit_tag_with("locked")
          survey.update :title => "new title 3", :changed_by => user
          survey.update :title => "new title 4", :changed_by => another_user
        end
  ...
  context "distinguishing between audited records" do
    ...
    it "should audit changes pertaining to each record only" do
      survey.update :title => "new title 1"
      another_survey.update :title => "another title 1"
  ...
  context "no changes on audited attributes" do
    ...
    it "should still store when only tag changes" do
      survey.audits.delete_all
      survey.update!(:audit_tag => 'one')
      survey.update!(:audit_tag => 'two')
      survey.update!(:audit_tag => 'three')
      survey.update!(:audit_tag => 'four')
  ...
  context "last change of a given attribute from past audits" do
    it "should return the last change of attribute even through the attribute hasn't changed in recent updates" do
      survey.update :title => "new title 1", :current_page => 1
      survey.update :title => "new title 2", :current_page => 2
      survey.update :title => "new title 3"
      survey.update :title => "new title 4"
      survey.update :title => "new title 5"
